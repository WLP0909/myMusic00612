...
<head>
    ...
    <meta name="description" content="一款基于HTML5、Css3的列表式音乐播放器，包含列表，音量，进度，时间以及模式等功能，不依赖任何库"><meta name="viewport" content="width=device-width, initial-scale=1, user.User-scalable=no">
    <meta name="viewport" content="width=device-width, initial-scale=1, user.User-scalable=no">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="renderer" content="webkit">
    <meta name="version" content="1.0.0">

    <link rel="stylesheet" href="static/css/userMusic.css"/>
...
<script>
    $().ready(function () {
        ...
        $.getJSON('/myMusic/selectUserMusic',{"id":id},function (data) {
            var musicList = [];
            var str = eval(data);
            for (var i = 0; i < str.length; i++) {
                musicList.push(
                {
                    title  : str[i].workName,
                    singer : str[i].userName,
                    cover  : str[i].photo,
                    src    : str[i].workAddress,
                    lyric  : str[i].workLyrics,
                })
            }
            new SMusic({
                musicList : musicList,
